...
const Page = () => {
    ...
    const [isConfirmEye, setIsConfirmEye] = useState(true);
    ...
    const handleConfirmEye = () => {
        setIsConfirmEye(!isConfirmEye);
    }

    useEffect(() => {
        const regex = /^(?!.*\s)(?=.*[^A-Za-z0-9]).+$/;
        if (username.length > 5 && password.length > 5 && regex.test(password) && password==confirmPassword) {
            setIsValidated(true);
        } else {
    ...
    return (
        <main className="grid grid-cols-2">
            ...
            <div className="flex flex-col items-center justify-between h-screen p-3">
                    ...
                    <form action="#" className="flex flex-col grow justify-around items-center" name="username">
                        ...
                        <div className="relative w-3/4">
                            ...
                            <div className="absolute right-2 top-1/2 transform -translate-y-1/2 cursor-pointer text-gray-500">
                                {isPasswordEye ? (
                                <EyeIcon className="h-4 w-4" onClick={handlePasswordEye} />
                                ) : (
                                <EyeSlashIcon className="h-4 w-4" onClick={handlePasswordEye} />
                                )}
                            </div>
                        </div>
                        <div className="relative w-3/4">
                            ...
                            <div className="absolute right-2 top-1/2 transform -translate-y-1/2 cursor-pointer text-gray-500">
                                {isConfirmEye ? (
                                <EyeIcon className="h-4 w-4" onClick={handleConfirmEye} />
                                ) : (
                                <EyeSlashIcon className="h-4 w-4" onClick={handleConfirmEye} />
                                )}
                            </div>
                        </div>
                        <button 
                ...
                <div className="flex">
                    <div className="text-sm p-3 text-gray-500">Already have an account?</div>
                    <Link href="/login" passHref>
                        <button type="button" className="bg-black py-3 px-5 rounded-3xl text-sm text-gray-100 cursor-pointer">Log in</button>
                    </Link>
                </div>
            </div>
        </main>
